...
This document provides Node/TypeScript-specific best practices and examples for implementing
MCP servers using the MCP TypeScript SDK. It covers project structure, server setup, tool
...
Use snake_case for tool names (e.g., "search_users", "create_project", "get_channel_info")
with clear, action-oriented names.
...
Error Handling:
  ...
  - Returns "No users found matching '<query>'" if empty`,
...
async function searchTool(params: SearchInput) {
  ...
  if (result.length > CHARACTER_LIMIT) {
    ...
    response.truncation_message =
      `Response truncated from ${data.length} to ` +
      `${truncatedData.length} items. ` +
  ...
  if (error instanceof AxiosError) {
    if (error.response) {
      switch (error.response.status) {
        ...
        case 403:
          return "Error: Permission denied. No access to this resource.";
        case 429:
          return "Error: Rate limit exceeded. Please wait.";
        default:
          return `Error: API failed with status ${error.response.status}`;
      }
 ...
 * including user search, project management, and data export.
 */

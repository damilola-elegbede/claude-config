...
if [[ "$*" == *"--list"* ]]; then
  ...
  SKILL_NAME=$(echo "$*" | sed 's/.*--preview[[:space:]]\+//' | awk '{print $1}')
elif [[ "$*" == *"--refresh"* ]]; then
...
if [[ "$MODE" == "preview" ]]; then
  ...
  echo ""
  echo "📖 Skill Preview: $SKILL_NAME"
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo ""
  echo "Source: anthropics/skills"

  # Get file list
  FILES=$(gh api "repos/anthropics/skills/contents/skills/$SKILL_NAME" 2>/dev/null)
  if [[ $? -ne 0 ]]; then
    echo "❌ Skill '$SKILL_NAME' not found"
    exit 1
  fi

  FILE_LIST=$(echo "$FILES" | jq -r '.[].name' | tr '\n' ', ' | sed 's/,$//')
  FILE_COUNT=$(echo "$FILES" | jq '. | length')
  echo "Files: $FILE_LIST ($FILE_COUNT files)"
  echo ""

  # Fetch and display SKILL.md
  ...
  echo ""
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo "💡 To install: /skills-import, then select \"$SKILL_NAME\""
  exit 0
fi
```
...
echo ""
echo "Enter skill numbers (comma-separated, or 'all'): "

# Use AskUserQuestion tool for selection
# Claude will prompt user and receive response

# After receiving selection, parse and download
SELECTED="$USER_INPUT"  # From AskUserQuestion

if [[ "$SELECTED" == "all" ]]; then
  SKILL_NAMES=$(echo "$SKILLS_JSON" | jq -r '.skills[].name')
else
  # Parse comma-separated numbers to skill names
  SKILL_NAMES=""
  for NUM in $(echo "$SELECTED" | tr ',' ' '); do
    NAME=$(echo "$SKILLS_JSON" | jq -r ".skills[$((NUM-1))].name")
...
for SKILL in $SKILL_NAMES; do
  ...
  if [[ -d "$INSTALLED_DIR/$SKILL" ]]; then
    echo "⚠️ $SKILL already installed. Overwrite? (y/n)"
  ...
  echo "✅ Installed $SKILL ($FILE_COUNT files)"
done
